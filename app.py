...
def upload_to_imgbb(file_bytes, name="photo"):
    """Upload bytes vers ImgBB, retourne l'URL ou None."""
    ...
    try:
        ...
        resp = requests.post(
            ...
            data={"key": IMGBB_API_KEY, "image": encoded,
                  "name": f"portfolio_{name}_{uuid.uuid4().hex[:6]}"},
            timeout=60
        )
        data = resp.json()
        return data["data"]["url"] if data.get("success") else None
    except Exception as e:
        app.logger.error(f"ImgBB error: {e}")
        return None
...
def save_image(file, folder=None, name="photo"):
    ...
    file.seek(0)
    img_bytes = file.read()
    url = upload_to_imgbb(img_bytes, name)
    if url:
        return url
    # Fallback local
...
# ─────────────────────────────────────────────
#  API — Upload clé ImgBB (client-side upload)
# ─────────────────────────────────────────────

@app.route("/api/upload-key")
@login_required
def get_upload_key():
    """Retourne la clé ImgBB aux admins connectés pour upload côté client."""
    if not IMGBB_API_KEY:
        return jsonify({"error": "IMGBB_API_KEY non configuré"}), 500
    return jsonify({"key": IMGBB_API_KEY})
